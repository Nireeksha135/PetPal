...
import { ArrowLeft, Pencil, Trash2, Bug } from "lucide-react";
import { Calendar, Tag, User as UserIcon, Bell } from "lucide-react";
import { useFleaTickTreatment } from "@/hooks/useFleaTick";
import { useDeleteFleaTick } from "@/hooks/useSaveFleaTick";
...
export default function FleaTickDetail() {
  ...
  const { data: pet } = usePet(treatment?.petId);
  const deleteMutation = useDeleteFleaTick();
  ...
  const handleDelete = () => {
    deleteMutation.mutate(
      { treatmentId: treatment.id, petId: treatment.petId },
      { onSuccess: () => navigate("/flea-tick", { replace: true }) },
    );
  };
      ...
      <div className="rounded-2xl bg-card p-8 shadow-card">
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-start">
          ...
          <div className="flex gap-3">
            <Link to={`/flea-tick/${treatment.id}/edit`}>
      ...
      <div className="rounded-2xl bg-card p-6 shadow-card">
        ...
        <div className="divide-y divide-border">
          <PetInfoRow
            ...
            value={treatmentTypeLabels[treatment.treatmentType]}
          />
          <PetInfoRow
            icon={Calendar}
            label="Date Applied"
            value={formatDate(treatment.dateApplied)}
          />
          <PetInfoRow
            icon={Calendar}
            label="Next Due Date"
            value={formatDate(treatment.nextDueDate)}
          />
          <PetInfoRow
            icon={UserIcon}
            label="Administered By"
            value={treatment.administeredBy ?? "Not recorded"}
          />
          <PetInfoRow
            icon={Bell}
            label="Reminder"
            value={treatment.reminderEnabled ? "Enabled" : "Disabled"}
          />
        </div>
      </div>
